#!/usr/bin/env python3
"""
Windows pip/venv Dependency installation script for Video to Transcription Service
Automatically detects CUDA version and installs appropriate PyTorch
Uses traditional pip and venv for package management on Windows
...
def detect_package_manager():
    """Windows pip/venv package manager detection"""
...
def get_best_python_executable():
    ...
    for python_cmd in python_candidates:
        try:
            # Handle py launcher commands differently
            if python_cmd.startswith('py '):
                cmd_parts = python_cmd.split()
                result = subprocess.run(cmd_parts + ['--version'], capture_output=True, text=True, check=True)
            else:
...
def setup_venv():
    ...
    try:
        ...
        if python_executable.startswith('py '):
            cmd_parts = python_executable.split()
            result = subprocess.run(cmd_parts + ['-m', 'venv', 'venv'], check=True)
        else:
            result = subprocess.run([python_executable, "-m", "venv", "venv"], check=True)
...
def get_venv_python():
    """Get path to Python executable in virtual environment for Windows"""
    venv_scripts = Path("venv") / "Scripts"
    
    # Try different possible Python executable names in the venv
    candidates = ["python.exe", "python3.exe", "python3.11.exe", "python3.12.exe", "python3.13.exe"]
    
    for candidate in candidates:
        python_path = venv_scripts / candidate
...
def get_venv_pip():
    """Get path to pip executable in virtual environment for Windows"""
    venv_scripts = Path("venv") / "Scripts"
    
    # Try different possible pip executable names in the venv
    candidates = ["pip.exe", "pip3.exe", "pip3.11.exe", "pip3.12.exe", "pip3.13.exe"]
    
    for candidate in candidates:
        pip_path = venv_scripts / candidate
        if pip_path.exists():
            return str(pip_path)
    
    # Fallback to the standard name
    return str(venv_scripts / "pip.exe")
...
def detect_cuda_version():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # If nvcc fails, try nvidia-ml-py
            try:
                python_cmd = get_python_command()
                ml_result = run_command(python_cmd + ["-c", """
try:
    import pynvml
    pynvml.nvmlInit()
    ...
    if "525" in driver_version or "530" in driver_version:
        print("CUDA: 12.0")
    elif "535" in driver_version or "545" in driver_version:
        print("CUDA: 12.2")
    else:
        print("CUDA: 12.1")  # Default for newer drivers
except:
    print("CUDA: 12.1")  # Safe default
"""], check=False)
                if ml_result.returncode == 0:
                    for line in ml_result.stdout.split('\n'):
                        if 'CUDA:' in line:
                            cuda_version = line.split('CUDA:')[1].strip()
                            logger.info(f"CUDA {cuda_version} detected via driver mapping")
                            return cuda_version
            except:
                pass
            
            # Default CUDA version for Windows with NVIDIA GPU
...
import torch
print(f'SUCCESS: PyTorch {torch.__version__} verified')
        ...
        if model_dir_pattern in item:
            ...
            if os.path.isdir(model_path):
                ...
                if os.path.exists(snapshots_dir) and os.listdir(snapshots_dir):
                    print(f"SUCCESS: Model {{model_name}} found in cache")
                    exit(0)
    
    print(f"ERROR: Model {{model_name}} not found in cache")
    exit(1)
    
except Exception as e:
    print(f"ERROR: Error checking model cache: {{e}}")
    exit(1)
"""]
...
try:
    ...
    print(f"SUCCESS: Model {model_name} downloaded and verified successfully")
except Exception as e:
...
def check_ffmpeg():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            logger.info("   Or use: choco install ffmpeg")
            return False
...
if 'venv' in sys.executable:
    print(f"SUCCESS: Using virtual environment: {sys.executable}")
else:
    print(f"WARNING: Not in virtual environment: {sys.executable}")
"""]
...
def main():
    """Main installation process for Windows pip/venv"""
    logger.info("=== Video to Transcription Service - Windows pip/venv Installation ===")
...
print('SUCCESS: All core dependencies verified')
print(f'PyTorch version: {torch.__version__}')
print(f'CUDA available: {torch.cuda.is_available()}')
print(f'faster-whisper available: {faster_whisper.__version__}')
"""]
        result = run_command(test_cmd, show_progress=True, capture_output=False)
        if result.returncode == 0:
    ...
    logger.info("📦 Package Manager: pip (virtual environment) for Windows")
    logger.info(f"🐍 Python: {get_venv_python()}")
    logger.info("💡 Remember to activate the virtual environment before running commands, venv/Scripts/activate")
